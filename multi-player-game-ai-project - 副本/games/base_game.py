...
class BaseGame(ABC):
    ...
    def record_move(self, player: int, action: Any, result: Dict[str, Any]|None = None):
        """记录移动"""
        move_record = {
            'player': player,
            'action': action,
    ...
    def get_legal_actions(self, player: int|None = None) -> List[Any]:
        """获取合法动作（别名）"""
