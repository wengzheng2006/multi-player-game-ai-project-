...
class GomokuGame(BaseGame):
    ...
    def step(self, action: Tuple[int, int]) -> Tuple[Dict[str, Any], float, bool, Dict[str, Any]]:
        ...
        if (row < 0 or row >= self.board_size or 
            ...
            return self.get_state(), -1, True, {'error': 'Invalid move: out of bounds'}
        
        # 检查位置是否已被占用
        if self.board[row, col] != 0:
            return self.get_state(), -1, True, {'error': 'Invalid move: position occupied'}
    ...
    def get_valid_actions(self, player: int = 1) -> List[Tuple[int, int]]:
        """获取有效动作列表"""
