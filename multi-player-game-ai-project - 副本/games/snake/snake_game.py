...
class SnakeGame(BaseGame):
    ...
    def step(self, action: Tuple[int, int]) -> Tuple[Dict[str, Any], float, bool, Dict[str, Any]]:
        ...
        # 检查游戏结束条件 - 只要有一个蛇死亡就结束
        done = not (self.alive1 and self.alive2)
    ...
    def get_valid_actions(self, player: int|None = None) -> List[Tuple[int, int]]:
        """获取有效动作列表"""
        # 四个方向：上、下、左、右
    ...
    def is_terminal(self) -> bool:
        """检查游戏是否结束"""
        return not (self.alive1 and self.alive2)  # 只要有一个蛇死亡就结束
